...
$("#district-select").on("change", function (event) {
    ...
    if (value == -1) {
        ...
        fetch_wards_data(
            (district = value),
            (callback_func = render_list_wards)
        );
    }
});
...
$("#type-select").on("change", function (event) {
    ...
    $.ajax("/get_sub_type", {
        ...
        success: function (data, status, xhr) {
            ...
            $("#area-type").html(
                `<option selected value="-1">Phân khúc diện tích</option>`
            );
            $("#price-type").html(
                `<option selected value="-1">Phân khúc giá cả</option>`
            );
            ...
            for (let i = 0; i < num_result; i++) {
                content =
                    content + `<option value="${i}">${area_type[i]}</option>`;
            }
            ...
            for (let i = 0; i < num_result; i++) {
                content =
                    content + `<option value="${i}">${price_type[i]}</option>`;
            }
...
function plot_num_items_and_price_avg_region(
    add_col,
    add_name,
    num_idx_ele,
    price_idx_ele
...
function compute_num_items_and_price_avg(group_col, keep_min_price = false) {
    ...
    if (keep_min_price) {
        sub_df = df.groupby(group_col).agg({
            sum_area: "sum",
            sum_price: "sum",
            count: "sum",
            property_min_price: "mean",
        });
    } else {
        sub_df = df
            .groupby(group_col)
            .agg({ sum_area: "sum", sum_price: "sum", count: "sum" });
    }
    let x = sub_df.apply((row) => parseInt(row[g_len + 1] / row[g_len]), {
        axis: 0,
    });
    sub_df.addColumn({ column: "Giá TB (/m2)", values: x, inplace: true });
    sub_df.addColumn({
        column: `Số ${LABEL}`,
        values: sub_df.column("count_sum"),
        inplace: true,
    });
    return sub_df;
}
...
function plot_num_items_and_price_avg_type(num_idx_ele, price_idx_ele) {
    ...
    sub_df.addColumn({
        column: " ",
        values: sub_df.column("property_type"),
        inplace: true,
    });
...
function plot_num_items_and_price_avg_time(num_idx_ele, price_idx_ele) {
    ...
    sub_df.addColumn({
        column: " ",
        values: sub_df.column("property_date"),
        inplace: true,
    });
...
function plot_num_items_and_price_area_sub_type(num_idx_ele, price_idx_ele) {
    ...
    sub_df.addColumn({
        column: " ",
        values: sub_df.column("property_area_type"),
        inplace: true,
    });
    sub_df.sort_values({ by: "property_min_area", inplace: true });

    $(`#plot_${num_idx_ele}_title`).html(
        `Số lượng ${LABEL} theo phân khúc diện tích`
    );
    sub_df
        .plot(`plot_${num_idx_ele}`)
        .line({ x: " ", y: `Số ${LABEL}`, displayModeBar: false });
    ...
    $(`#plot_${price_idx_ele}_title`).html(
        "Giá trung bình theo từng phân khúc diện tích"
    );
    sub_df
        .plot(`plot_${price_idx_ele}`)
        .line({ x: " ", y: "Giá TB (/m2)", displayModeBar: false });
...
$("#apply-filter").on("click", function (event) {
    ...
    $.ajax("/get_statistic", {
        ...
        error: function (jqXhr, textStatus, errorMessage) {
            data = {
                status: false,
                message: "Kết nối server thất bại",
                data: [],
            };
            render_statistical_data(data);
        },
